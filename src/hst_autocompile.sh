...
# set -e
# Autocompile Script for NusantaraCP package Files.
...
usage() {
	...
	echo "This would install a Nusantara Panel package compiled with the"
...
HESTIA_ARCHIVE_LINK='https://github.com/nusantara-panel/nusantara/archive/'$branch'.tar.gz'
if [[ $NGINX_V =~ - ]]; then
...
if [ "$HESTIA_B" = true ]; then
	...
	for BUILD_ARCH in $arch; do
		echo "Building Nusantara Panel package..."
