...
- In initial versions of NusantaraCP, we used Jailkit to enabled Jailed SSH. It had major disadvantages, so we have decided it to replace it with [bubblewrap](https://github.com/containers/bubblewrap). Users running Jailed SSH in the past are advised to run the migration script! It can be found in `/usr/local/hestia/install/upgrade/manual/migrate_jailkit_to_bubblewrap.sh`. See [#4698](https://github.com/nusantara-panel/nusantara/pull/4698)
...
- Include <https://github.com/nusantara-panel/nusantara/blob/main/install/upgrade/manual/secure_php.sh>
...
- NusantaraCP 1.7.2 fixes an issue with downloading certificates from Let's Encrypt due to the implementation of asynchronous requests which will go live on 10th April 2023. Please update your server before this date to ensure compatibility with Let's Encrypt.
...
- Fixed an issue in auto update script for NusantaraCP (#2967)
...
- Grey out phpmyadmin buttons + add link to docs.nusantarapanel.local for support (#2843)
...
- Fixed an issue where MariaDB installations were broken when performing a clean install of NusantaraCP v1.5.9 (#2452 | 2446)
...
- Fixed an issue with upgrade script causing command to to be executed (<https://forum.nusantarapanel.local/t/upgrading-to-1-5-5-error-line/5449/3>)
...
- Fixed an issue which impacted the performance of Nextcloud/Owncloud ([forum post](https://forum.nusantarapanel.local/t/tip-create-a-nginx-template-for-nextcloud-to-let-synchronize-files-bigger-than-10mb/5123))
- Fixed an issue where the access port for NusantaraCP was not properly set on install (#2288 / #2291)
...
- Updated command line examples for docs.nusantarapanel.local
...
- Fix issue with ipset not working properly [Forum](https://forum.nusantarapanel.local/t/error-ipset-object-not-found/5015)
...
- Adjust /etc/apt/sources.list.d/hestia.list to include architecture to resolve issue with I386 missing in apt.nusantarapanel.local
...
- **NOTE:** Hestia packages for arm64 has been added to atp.nusantarapanel.local please use the normal install instructions instead! For current ARM installs to enable auto update remove the `#` in /etc/apt/sources.list.d/hestia.list `# deb https://apt.nusantarapanel.local/ focal main` becomes `deb https://apt.nusantarapanel.local/ focal main` and then run `apt update && apt upgrade -y`
- **NOTE:** Make sure your server / VPS has a valid PTR record or otherwise you will not be able to send any mail!

### Features

- Add support for Dovecote Sieve #2163 (@gejobj) => [How to enable Managesieve](https://nusantarapanel.local/docs/server-administration/email.html#how-can-i-enable-managesieve)
...
- Add small wait for /usr/bin/iptables-restore [Forum](https://forum.nusantarapanel.local/t/clean-install-arm64-does-not-start-after-reboot-v-start-service-iptables/4395/7) + Fixed v-add-firewall / v-delete-firewall function (#2112) @myrevery
...
- Synced up translations with NusantaraCP (IT, PL, RU, SK and ZN-CN updated)
...
- Fixed UI issues after upgrade jQuery + jQuery UI to last version (#2021 and #2032) + [forum](https://forum.nusantarapanel.local/t/confusion-about-send-welcome-email-checkbox/4259/11)
...
- Add support for automated testing for NusantaraCP code with @drone
...
- Keep changes made by /install/upgrade/manual/install_awstats_geopip.sh on update NusantaraCP (via Discord)
...
- Fixed an issue where the api was enabled after an update of NusantaraCP
...
- Updated APT repository key for PHP from packages.sury.org (<https://forum.nusantarapanel.local/t/apt-upgrade-failed-gpg-error-packages-sury-org>)
...
- Fixed an issue with passwords containing "`'`" - [Forum](https://forum.nusantarapanel.local/t/two-factor-authentication-issue-with-standard-user/1652/)
...
- Add ca-certificates, software-properties-common to the dependencies (#1073 + [Forum](https://forum.nusantarapanel.local/t/hestiscp-fails-on-new-debian-9-vps/1623/8)) - thanks **@daniel-eder**
...
- A newly redesigned user interface which features:
  - A softer color palette which better matches the Nusantara Panel logo colors.
...
[1.0.4]: https://github.com/nusantara-panel/nusantara/releases/tag/1.0.4
[1.0.3]: https://github.com/nusantara-panel/nusantara/releases/tag/1.0.3
[1.0.1]: https://github.com/nusantara-panel/nusantara/releases/tag/1.0.1
[1.0.0-190618]: https://github.com/nusantara-panel/nusantara/releases/tag/1.0.0-190618
[0.9.8-28]: https://github.com/nusantara-panel/nusantara/releases/tag/0.9.8-28
