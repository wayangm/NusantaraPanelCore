...
#===========================================================================#
#                                                                           #
# Nusantara Panel - Upgrade Function Library                           #
#                                                                           #
#===========================================================================#
...
upgrade_health_check() {
	...
	if [ -z "$VERSION" ]; then
		...
		echo
		echo "[ ! ] Unable to detect installed version of Nusantara Panel."
...
upgrade_welcome_message() {
	...
	echo "                                                                              "
	echo "                    Nusantara Panel Software Update                      "
...
upgrade_welcome_message_log() {
	echo "============================================================================="
	echo "Nusantara Panel Software Update Log"
	echo "============================================================================="
	echo
...
upgrade_complete_message() {
	...
	echo "https://github.com/nusantara-panel/nusantara/issues                                  "
	echo
	echo "Read the release notes to learn about new fixes and features:                "
	echo "https://github.com/nusantara-panel/nusantara/blob/release/CHANGELOG.md               "
	echo
	echo "We hope that you enjoy using this version of Nusantara Panel,           "
	echo "have a wonderful day!                                                        "
	echo
	echo "Sincerely,                                                                   "
	echo "The Nusantara Panel development team                                    "
	echo
	echo "Web:      https://www.nusantarapanel.local/                                          "
	echo "Docs:     https://docs.nusantarapanel.local/										   "
	echo "Forum:    https://forum.nusantarapanel.local/                                        "
	echo "GitHub:   https://github.com/nusantara-panel/nusantara/                              "
	echo
	echo "Help support the Nusantara Panel project by donating via PayPal:        "
	echo "https://www.nusantarapanel.local/donate                                              "
	echo
...
upgrade_get_version() {
	# Retrieve new version number for Nusantara Panel from .deb package
...
upgrade_send_notification_to_panel() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		...
		$BIN/v-add-user-notification "$ROOT_USER" 'Development snapshot installed' '<p><span class="u-text-bold">Version:</span> '$new_version'<br><span class="u-text-bold">Code Branch:</span> '$RELEASE_BRANCH'</p><p>Please report any bugs by <a href="https://github.com/nusantara-panel/nusantara/issues" target="_blank">opening an issue on GitHub</a>, and feel free to share your feedback on our <a href="https://forum.nusantarapanel.local" target="_blank">discussion forum</a>.</p><p><i class="fas fa-heart icon-red"></i> The Nusantara Panel development team</p>'
	elif [[ "$new_version" =~ "beta" ]]; then
		# Send feedback notification for beta releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Thank you for testing Nusantara Panel '$new_version'.' '<p>Please share your feedback with our development team through our <a href="https://forum.nusantarapanel.local" target="_blank">discussion forum</a>.</p><p>Found a bug? <a href="https://github.com/nusantara-panel/nusantara/issues" target="_blank">Open an issue on GitHub</a>!</p><p><i class="fas fa-heart icon-red"></i> The Nusantara Panel development team</p>'
	else
		# Send normal upgrade complete notification for stable releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Upgrade complete' '<p>Nusantara Panel has been updated to <span class="u-text-bold">v'$new_version'</span>.</p><p><a href="https://github.com/nusantara-panel/nusantara/blob/release/CHANGELOG.md" target="_blank">View release notes</a></p><p>Please report any bugs by <a href="https://github.com/nusantara-panel/nusantara/issues" target="_blank">opening an issue on GitHub</a>.</p><p class="u-text-bold">Have a wonderful day!</p><p><i class="fas fa-heart icon-red"></i> The Nusantara Panel development team</p>'
	fi
}
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		echo "$HOSTNAME has been upgraded from Nusantara Panel v$VERSION to v${new_version}." >> $message_tmp_file
		...
		echo "What's new: https://github.com/nusantara-panel/nusantara/blob/$RELEASE_BRANCH/CHANGELOG.md" >> $message_tmp_file
		echo >> $message_tmp_file
		echo "What to do if you run into issues:" >> $message_tmp_file
		echo "- Check our forums for possible solutions: https://forum.nusantarapanel.local" >> $message_tmp_file
		echo "- File an issue report on GitHub: https://github.com/nusantara-panel/nusantara/issues" >> $message_tmp_file
		echo "" >> $message_tmp_file
		echo "Help support the Nusantara Panel project by donating via PayPal: https://www.nusantarapanel.local/donate" >> $message_tmp_file
		echo "===================================================" >> $message_tmp_file
		echo "Have a wonderful day," >> $message_tmp_file
		echo "The Nusantara Panel development team" >> $message_tmp_file
...
upgrade_init_backup() {
	...
	# Nusantara Panel configuration files
...
upgrade_init_logging() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Development build for testing purposes only. Report bugs at https://github.com/nusantara-panel/nusantara/issues/."
	fi
	if [[ "$new_version" =~ "beta" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Beta release. Please report bugs at https://github.com/nusantara-panel/nusantara/issues/."
	fi
}
...
upgrade_start_routine() {
	...
	if [ $(check_version $latest_version) -gt $(check_version $VERSION) ]; then
		...
		echo ""
		echo "[ ! ] The latest version of Nusantara Panel is already installed."
...
upgrade_restart_services() {
	...
	# Always restart the Nusantara Panel service
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - hestia"
	fi
