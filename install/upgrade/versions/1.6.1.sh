...
# Nusantara Panel upgrade script for target version 1.6.1
...
check=$(cat /etc/apt/sources.list.d/hestia.list | grep "beta.nusantarapanel.local")
if [ ! -z "$check" ]; then
	echo "[ ! ] Change to stable release!"
	sed -i '/beta.nusantarapanel.local/d' /etc/apt/sources.list.d/hestia.list
	sed -i 's/#//g' /etc/apt/sources.list.d/hestia.list
fi
