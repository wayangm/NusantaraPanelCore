...
Hestia is an open-source project, and we welcome contributions from the community. Please read the [contributing guidelines](https://github.com/nusantara-panel/nusantara/blob/main/CONTRIBUTING.md) for additional information.
...
::: warning
Development builds are unstable. If you encounter a bug, please [report it via GitHub](https://github.com/nusantara-panel/nusantara/issues/new/choose) or [submit a Pull Request](https://github.com/nusantara-panel/nusantara/pulls).
:::
...
1. [Fork Hestia](https://github.com/nusantara-panel/nusantara/fork) and clone the repository to your local machine
...
1. Navigate to `/install` in the VM, then install Hestia with these flags
   ...
   ```bash
   cd ../install
   bash hst-install-ubuntu.sh --hostname demo.nusantarapanel.local --email admin@example.com --username admin --password Password123 --with-debs /tmp/hestiacp-src/deb/ --interactive no --force
   ```
...
Please refer to the [contributing guidelines](https://github.com/nusantara-panel/nusantara/blob/main/CONTRIBUTING.md#development-guidelines) for more details on submitting code changes for review.
...
git clone --recurse-submodules https://github.com/nusantara-panel/nusantara
