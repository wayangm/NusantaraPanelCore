...
echo "deb [arch=$ARCH signed-by=/usr/share/keyrings/hestia-beta-keyring.gpg] https://beta-apt.nusantarapanel.local/ $codename main" >> $apt/hestia.list
curl -s "https://beta-apt.nusantarapanel.local/pubkey.gpg" | gpg --dearmor | tee /usr/share/keyrings/hestia-beta-keyring.gpg > /dev/null 2>&1
...
```bash
# Debian
wget https://beta-apt.nusantarapanel.local/hst-install-debian.sh
# or Ubuntu
wget https://beta-apt.nusantarapanel.local/hst-install-ubuntu.sh
```
...
Edit `/etc/apt/sources.list.d/hestia.list` and remove the `#` in front of `apt.nusantarapanel.local`, and add a `#` in front of `beta-apt.nusantarapanel.local`.

Once that’s done, run `apt update && apt upgrade` to rollback to the regular release.

## Reporting bugs

If you encounter a bug, please [open an issue](https://github.com/nusantara-panel/nusantara/issues/new/choose) or [submit a Pull Request](https://github.com/nusantara-panel/nusantara/pulls). You can also report it on our forum or our Discord server
